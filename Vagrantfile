...
modx_download_link = configuration['Modx']['downloadLink'] ||= 'https://modx.com/download/direct?id=modx-2.5.4-pl.zip'

unless system("
		if [ #{ARGV[0]} = 'up' -o #{ARGV[0]} = 'reload' ]; then
			#{File.dirname(__FILE__)}/utils/provision/beforeStart.sh #{File.dirname(__FILE__)} #{modx_download_link}
		fi
	")
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
	...
	config.vm.synced_folder configuration['Mount']['from'] ||= 'www', '/var/www',
													id: 'modx', type: 'nfs', mount_options: ['rw', 'vers=3', 'udp', 'noatime', 'actimeo=1']
	...
	config.vm.provider 'virtualbox' do |vb|
		...
		vb.customize ['modifyvm', :id, '--groups', configuration['VirtualMachine']['group'] ||= '/vagrant/modx']
	...
	config.vm.provision :puppet do |puppet|
		...
		puppet.facter = {
				...
				:aliases => configuration['VirtualMachine']['aliases'] ||= [],
	...
	config.vm.provision 'after-start', type: 'shell', path: 'utils/provision/afterStart.sh', args: ['/var/www'], :privileged => false, :run => 'always'
